...
public class Dust  extends PApplet{
	...
	boolean flagIsCalibrating = false;
	...
	DustImage dust;
	float[] syphonUV = {0.30f, 0.0f, 0.69f, 0.9f}; //x1, y1, x2, y2 from upper left corner!!!
	float[] dustUV = {0.0f, 0.12f, 1.0f, 1.0f}; //x1, y1, x2, y2 from upper left corner!!!
	...
	public void setup() {		    
		...
		defaultImg = new PImage[1];
		
		defaultImg[0] = loadImage("dust0.png");

		dust = new DustImage(WIDTH, HEIGHT,RGB);
		displayLog("starting UP...");
	}
	...
	public void draw() {
		...
		canvas.background(0);
		drawSimulation();
		if(flagIsCalibrating){
			for(int i = 0; i < 2; i ++){
				for(int j = 0; j < 4; j++){
					canvas.fill(i*255, j*255, (3-j)*255);
					canvas.rect(i*width/2, j*height/4, width/2, height/4);
				}
			}
		}
		
		if(!player.isInSimulationMode()){
			canvas.fill(255);
			canvas.text(message, 240, 490);
		}
		...
		for(int i = 0; i < logQueue.length; i++){
			fill(200 - i * 10);
			text(logQueue[i], 10, 600 + i * 15);
		}
	}
	
	void displayLog(String message){
		for(int i = (logQueue.length - 1); i > 0; i--){
			if(logQueue[i-1] != null){
	...
	void captureShadow(){
		if (client.available()&&!player.isInSetupMode()){
			...
			setupSimulation();
		}else if(player.isInSetupMode()){
			dust.clear();
			dust.copy(defaultImg[0], 
					0, 
					0, 
					defaultImg[0].width, 
					defaultImg[0].height, 
	...
	public void keyPressed()
	{
		...
		if( key == 'c') {
			flagIsCalibrating = !flagIsCalibrating;
		} 
		if( key == 'b') {
			this.globalBlend.start(1, -1);
			this.led.start(1, 1);
		} 
	...
	class Player{
		...
		boolean isInSimulationMode(){
			return (playmode == PMODE_SIMULATION)? true: false;
		}

		boolean isInSetupMode(){
			return (playmode == PMODE_SETUP)? true: false;
		}
		...
		void update(){
			...
			case PMODE_SETUP:
				if(isItTime(1)){
					...
					resetTrigger();
					displayLog("Playmode = PMODE_SETUP_KIN_STARTED");
				}
				break;
			case PMODE_SETUP_KIN_STARTED:
				if(trigger_kinect_started){
					...
					globalBlend.start(10, -1);
					resetTrigger();
					displayLog("Playmode = PMODE_TRIGGER_PERSON");
				}else if(isItTime(30)){
					displayLog("kinect failed to start, restart again");
					playmode = PMODE_SETUP;
					resetTrigger();
					displayLog("Playmode = PMODE_SETUP");
				}
				break;
			case PMODE_TRIGGER_PERSON:
				if(trigger_person){
					...
					resetTrigger();
					displayLog("Playmode = PMODE_START_LED");
				}else if(isItTime(30)){
					displayLog("kinect might not work anymore, restart again");
					playmode = PMODE_SETUP;
					resetTrigger();
					displayLog("Playmode = PMODE_SETUP");
				}
			...
			case PMODE_START_LED:
				...
				resetTrigger();
				displayLog("Playmode = PMODE_RESTART_KINECT");
				break;
			case PMODE_RESTART_KINECT:
				killAndRestartKinect();
				playmode = PMODE_TRIGGER_KIN_RESTARTED;
				resetTrigger();
				displayLog("Playmode = PMODE_TRIGGER_KIN_RESTARTED");
				break;
			case PMODE_TRIGGER_KIN_RESTARTED:
				if(trigger_kinect_started){
					playmode = PMODE_SETUP_SIMULATION;
					resetTrigger();
					displayLog("Playmode = PMODE_SETUP_SIMULATION");
				}else if(isItTime(20)){
					displayLog("kinect might not work anymore, restart again");
					playmode = PMODE_RESTART_KINECT;
					resetTrigger();
			...
			case PMODE_SETUP_SIMULATION:
				if(isItTime(0.2f)){
					...
					resetTrigger();
					displayLog("Playmode = PMODE_SIMULATION");
				}
				break;
			case PMODE_SIMULATION:
				if(isItTime(40)){
					...
					resetTrigger();
					displayLog("Playmode = PMODE_DISSOLVE");
				}
				break;
			case PMODE_DISSOLVE:
				if(isItTime(1)){
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "Dust" };
		if (passedArgs != null) {
